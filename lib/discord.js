// Talks to Discord's REST API directly using the bot token.
...
// Find a Pixel Villa guild member by their Discord username.
export async function getMemberInfoByUsername(username) {
  try {
    ...
    // Discord's guild member search endpoint.
    const res = await discordFetch(
    ...
    const roles = member.roles || [];
    const actualUsername = member.user.username;
    ...
    console.error("getMemberInfoByUsername error:", error);
    return { isMember: false };
  }
}
